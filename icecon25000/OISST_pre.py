...
# 文件路径
base_folder_path  = r"D:\zmh\icecon25000_data\OISST"   # OISST数据的根目录
input_path_north = os.path.join(base_folder_path, "north")  
input_path_south = os.path.join(base_folder_path, "south")  
...
def process_daily_nc(input_dir: str, hemisphere: str, crs_ps: CRS, grid_x_hem: np.ndarray, grid_y_hem: np.ndarray, output_dir: str):
    ...
    for file in tqdm(all_files, desc=f"Processing {hemisphere} hemisphere", unit="file"):
        try:
            ...
            # 保存为 NetCDF
            output_file = os.path.join(output_dir, f"OISST_{hemisphere}_icecon_25000_{file[-11:-3]}.nc")
...
if __name__ == "__main__":
    # --- 步骤5：执行处理 ---
    process_daily_nc(input_path_north, 'north', crs_psn, grid_x_n, grid_y_n, final_output_dir_north) # 处理北极每日数据
    process_daily_nc(input_path_south, 'south', crs_pss, grid_x_s, grid_y_s, final_output_dir_south) # 处理南极每日数据
