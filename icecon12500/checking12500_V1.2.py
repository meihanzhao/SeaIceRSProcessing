...
def plot_combined_comparison_and_error(data1, data2, relative_error, x_coords, y_coords, proj_crs, extent, title, output_path, vmin, vmax, cmap, date_str=None):
    """
    绘制组合图，包含三个子图：左侧为融合数据，中间为相对误差，右侧为检验数据
    """
    try:
        if proj_crs == ccrs.NorthPolarStereo(central_longitude=-45):
            proj = pyproj.Proj(proj='stere', lat_ts=70, lat_0=90, lon_0=-45)
        ...
        seaice_colors = ["#f0f9e8", "#bae4bc", "#7bccc4", "#43a2ca", "#0868ac", "#084081"]
        seaice_cmap = LinearSegmentedColormap.from_list("SeaIceCmap", seaice_colors)

        error_colors = ["darkblue", "blue", "deepskyblue", "cyan", "limegreen", "yellow", "red", "firebrick"]
        error_cmap = ListedColormap(error_colors)
        error_bounds = [0, 5, 10, 15, 20, 25, 30, 35, 40]
        error_norm = BoundaryNorm(error_bounds, error_cmap.N)

        fig, axs = plt.subplots(1, 3, figsize=(18, 6), subplot_kw={'projection': proj_crs})

        # 绘制融合数据、相对误差和检验数据
        for i, (data, ax, cmap, norm, label) in enumerate(zip([data1, relative_error, data2], axs, [seaice_cmap, error_cmap, seaice_cmap], [None, error_norm, None], ['海冰密集度 (%)', '相对误差 (%)', '海冰密集度 (%)'])):
            ax.set_extent(extent, crs=ccrs.PlateCarree())
            ...
            im = ax.pcolormesh(
                lon, lat, data,
                transform=ccrs.PlateCarree(), cmap=cmap, shading='auto', vmin=vmin if i != 1 else None, vmax=vmax if i != 1 else None, norm=norm if i == 1 else None
            )
            ...
            ax.set_title(f"{'融合数据' if i == 0 else '相对误差' if i == 1 else '检验数据'}")
        ...
        print(f"Error generating combined comparison and error plot: {e}")

def plot_combined_data_and_error(data, relative_error, x_coords, y_coords, proj_crs, extent, title, output_path, vmin, vmax, cmap, date_str=None):
    """
    绘制融合数据和相对误差数据的组合图，包含两个子图：左侧为融合数据，右侧为相对误差
    """
    try:
        if proj_crs == ccrs.NorthPolarStereo(central_longitude=-45):
            proj = pyproj.Proj(proj='stere', lat_ts=70, lat_0=90, lon_0=-45)
        ...
        # 绘制融合数据和相对误差
        for i, (d, ax, cmap, norm, label) in enumerate(zip([data, relative_error], axs, [seaice_cmap, error_cmap], [None, error_norm], ['海冰密集度 (%)', '相对误差 (%)'])):
            ax.set_extent(extent, crs=ccrs.PlateCarree())
            ...
            im = ax.pcolormesh(
                lon, lat, d,
                transform=ccrs.PlateCarree(), cmap=cmap, shading='auto', vmin=vmin if i == 0 else None, vmax=vmax if i == 0 else None, norm=norm if i == 1 else None
            )
            ...
            ax.set_title(f"{'融合数据' if i == 0 else '相对误差'}")
        ...
        print(f"Error generating combined data and error plot: {e}")

def plot_combined_data_and_error_with_regions(data, relative_error, x_coords, y_coords, proj_crs, extent, title, output_path, vmin, vmax, cmap, date_str=None, sea_regions=None, hemisphere=None):
    """
    绘制融合数据和相对误差数据的组合图，包含两个子图：左侧为融合数据，右侧为相对误差
    并在图上标注14个重点海域
    """
    try:
        ...
        for i, (d, ax, cmap, norm, label) in enumerate(zip([data, relative_error], axs, [seaice_cmap, error_cmap], [None, error_norm], ['海冰密集度 (%)', '相对误差 (%)'])):
            ...
            # 绘制重点海域
            if i == 0 and sea_regions is not None:  # 只在融合数据子图上绘制
                for region_name, region_polygon in sea_regions.items():
                    # 判断当前海域是否属于当前半球
                    if (hemisphere == 'north' and 'North' in region_name) or \
                       (hemisphere == 'south' and 'South' in region_name):
                        # 将经纬度坐标转换为投影坐标
                        region_polygon = np.array(region_polygon)
                        x_poly, y_poly = proj(region_polygon[:, 0], region_polygon[:, 1])
                        
                        # 1. 闭合多边形 Close the polygon
                        x_poly = np.append(x_poly, x_poly[0])
                        y_poly = np.append(y_poly, y_poly[0])

                        # 2. 调整线宽 Adjust linewidth
                        ax.plot(x_poly, y_poly, color='red', linewidth=1.5, transform=proj_crs)  # 绘制多边形

                        # 3. 调整标签 Remove "北极/南极" and "_", change font size and color
                        region_name_chinese = region_name.replace(" ", "").replace("North", "").replace("South", "").replace("Sea","海").replace("Ocean","洋").replace("CentralArctic", "中央北冰洋").replace("CanadianArchipelago", "加拿大群岛").replace("Barents", "巴伦支").replace("Kara", "喀拉").replace("Laptev", "拉普捷夫").replace("EastSiberian", "东西伯利亚").replace("Chukchi", "楚科奇").replace("Beaufort", "波弗特").replace("Weddell", "威德尔").replace("Indian", "印度").replace("Pacific", "太平洋").replace("Ross", "罗斯").replace("Bellingshausen", "别林斯高晋").replace("Amundsen", "阿蒙森").replace("Bay","湾").replace("Bering","白令").replace("Greenland","格陵兰").replace("Cosmonauts", "宇航员").replace("Prydz", "普里兹")
                        ax.text(np.mean(x_poly), np.mean(y_poly), region_name_chinese, fontsize=6, color='darkred', ha='center', va='center', transform=proj_crs) # 添加中文标签
        ...
        print(f"Error generating combined data and error plot with regions: {e}")
...
def process_files(fusion_path, test_path, var_fusion, var_test, hemisphere, sea_regions, results_base_path):
    ...
    for fusion_file, test_file in zip(fusion_files, test_files):
        ...
        # 绘制每日相对误差分布图
        output_dir_daily = os.path.join(results_base_path, hemisphere, '相对误差分布_日')
        ...
        # 绘制融合数据与检验数据对比图
        output_dir_comparison = os.path.join(results_base_path, hemisphere, '融合数据与检验数据分布图_日')
        os.makedirs(output_dir_comparison, exist_ok=True)
        output_path_comparison = os.path.join(output_dir_comparison, f"{hemisphere_chinese}_融合数据与检验数据_{date_str}.png")
        ...
        # 绘制组合的比较图和误差图
        output_dir_combined = os.path.join(results_base_path, hemisphere, '融合数据与相对误差与检验数据分布图_日')
        os.makedirs(output_dir_combined, exist_ok=True)
        output_path_combined = os.path.join(output_dir_combined, f"{hemisphere_chinese}_融合数据与相对误差与检验数据_{date_str}.png")
        plot_combined_comparison_and_error(fusion_data[0], test_data[0], relative_error[0], x_coords_2d, y_coords_2d, proj_crs, extent,
                                           f'{hemisphere_chinese} - {date_str}', output_path_combined, 0, 100, seaice_cmap, date_str)

        # 绘制融合数据和误差数据的组合图
        output_dir_combined_data_error = os.path.join(results_base_path, hemisphere, '融合数据与相对误差分布图_日')
        os.makedirs(output_dir_combined_data_error, exist_ok=True)
        output_path_combined_data_error = os.path.join(output_dir_combined_data_error, f"{hemisphere_chinese}_融合数据与相对误差_{date_str}.png")
        plot_combined_data_and_error(fusion_data[0], relative_error[0], x_coords_2d, y_coords_2d, proj_crs, extent,
                                      f'{hemisphere_chinese} - {date_str}', output_path_combined_data_error, 0, 100, seaice_cmap, date_str)
        
        # 绘制融合数据和误差数据的组合图，并标注重点海域
        output_dir_combined_data_error_with_regions = os.path.join(results_base_path, hemisphere, '融合数据与相对误差分布图_重点海域_日')
        os.makedirs(output_dir_combined_data_error_with_regions, exist_ok=True)
        output_path_combined_data_error_with_regions = os.path.join(output_dir_combined_data_error_with_regions, f"{hemisphere_chinese}_融合数据与相对误差_重点海域_{date_str}.png")
        plot_combined_data_and_error_with_regions(fusion_data[0], relative_error[0], x_coords_2d, y_coords_2d, proj_crs, extent,
                                                f'{hemisphere_chinese} - {date_str}', output_path_combined_data_error_with_regions, 0, 100, seaice_cmap, date_str, sea_regions, hemisphere) # 关键修改：添加 hemisphere 参数
        ...
        if date.day == 1:
            ...
            if np.all(np.isnan(monthly_fusion_data)) or np.all(np.isnan(monthly_test_data)):
                ...
                # 绘制月度相对误差分布图
                output_dir_monthly = os.path.join(results_base_path, hemisphere, '相对误差分布_月')
                ...
                # 绘制融合数据与检验数据对比图
                output_dir_comparison_monthly = os.path.join(results_base_path, hemisphere, '融合数据与检验数据分布图_月')
                os.makedirs(output_dir_comparison_monthly, exist_ok=True)
                output_path_comparison_monthly = os.path.join(output_dir_comparison_monthly, f"{hemisphere_chinese}_融合数据与检验数据_{month_str}.png")
                ...
                # 绘制组合的比较图和误差图
                output_dir_combined_monthly = os.path.join(results_base_path, hemisphere, '融合数据与相对误差与检验数据分布图_月')
                os.makedirs(output_dir_combined_monthly, exist_ok=True)
                output_path_combined_monthly = os.path.join(output_dir_combined_monthly, f"{hemisphere_chinese}_融合数据与相对误差与检验数据_{month_str}.png")
                plot_combined_comparison_and_error(monthly_fusion_avg, monthly_test_avg, monthly_relative_error, x_coords_2d, y_coords_2d, proj_crs, extent,
                                                   f'{hemisphere_chinese} - {month_str}', output_path_combined_monthly, 0, 100, seaice_cmap, month_str)
                # 绘制融合数据和误差数据的组合图
                output_dir_combined_data_error_monthly = os.path.join(results_base_path, hemisphere, '融合数据与相对误差分布图_月')
                os.makedirs(output_dir_combined_data_error_monthly, exist_ok=True)
                output_path_combined_data_error_monthly = os.path.join(output_dir_combined_data_error_monthly, f"{hemisphere_chinese}_融合数据与相对误差_{month_str}.png")
                plot_combined_data_and_error(monthly_fusion_avg, monthly_relative_error, x_coords_2d, y_coords_2d, proj_crs, extent,
                                              f'{hemisphere_chinese} - {month_str}', output_path_combined_data_error_monthly, 0, 100, seaice_cmap, month_str)
                
                # 绘制融合数据和误差数据的组合图，并标注重点海域
                output_dir_combined_data_error_with_regions_monthly = os.path.join(results_base_path, hemisphere, '融合数据与相对误差分布图_重点海域_月')
                os.makedirs(output_dir_combined_data_error_with_regions_monthly, exist_ok=True)
                output_path_combined_data_error_with_regions_monthly = os.path.join(output_dir_combined_data_error_with_regions_monthly, f"{hemisphere_chinese}_融合数据与相对误差_重点海域_{month_str}.png")
                plot_combined_data_and_error_with_regions(monthly_fusion_avg, monthly_relative_error, x_coords_2d, y_coords_2d, proj_crs, extent,
                                                        f'{hemisphere_chinese} - {month_str}', output_path_combined_data_error_with_regions_monthly, 0, 100, seaice_cmap, month_str, sea_regions, hemisphere)
                ...
                if date.month == 1:
                    ...
                    # 绘制年度相对误差分布图
                    output_dir_yearly = os.path.join(results_base_path, hemisphere, '相对误差分布_年')
                    ...
                    # 绘制融合数据与检验数据对比图
                    output_dir_comparison_yearly = os.path.join(results_base_path, hemisphere, '融合数据与检验数据分布图_年')
                    os.makedirs(output_dir_comparison_yearly, exist_ok=True)
                    output_path_comparison_yearly = os.path.join(output_dir_comparison_yearly, f"{hemisphere_chinese}_融合数据与检验数据_{year_str}.png")
                    ...
                    # 绘制组合的比较图和误差图
                    output_dir_combined_yearly = os.path.join(results_base_path, hemisphere, '融合数据与相对误差与检验数据分布图_年')
                    os.makedirs(output_dir_combined_yearly, exist_ok=True)
                    output_path_combined_yearly = os.path.join(output_dir_combined_yearly, f"{hemisphere_chinese}_融合数据与相对误差与检验数据_{year_str}.png")
                    plot_combined_comparison_and_error(yearly_fusion_avg, yearly_test_avg, yearly_relative_error, x_coords_2d, y_coords_2d, proj_crs, extent,
                                                       f'{hemisphere_chinese} - {year_str}', output_path_combined_yearly, 0, 100, seaice_cmap, year_str)

                    # 绘制融合数据和误差数据的组合图
                    output_dir_combined_data_error_yearly = os.path.join(results_base_path, hemisphere, '融合数据与相对误差分布图_年')
                    os.makedirs(output_dir_combined_data_error_yearly, exist_ok=True)
                    output_path_combined_data_error_yearly = os.path.join(output_dir_combined_data_error_yearly, f"{hemisphere_chinese}_融合数据与相对误差_{year_str}.png")
                    plot_combined_data_and_error(yearly_fusion_avg, yearly_relative_error, x_coords_2d, y_coords_2d, proj_crs, extent,
                                                  f'{hemisphere_chinese} - {year_str}', output_path_combined_data_error_yearly, 0, 100, seaice_cmap, year_str)
                    
                    # 绘制融合数据和误差数据的组合图，并标注重点海域
                    output_dir_combined_data_error_with_regions_yearly = os.path.join(results_base_path, hemisphere, '融合数据与相对误差分布图_重点海域_年')
                    os.makedirs(output_dir_combined_data_error_with_regions_yearly, exist_ok=True)
                    output_path_combined_data_error_with_regions_yearly = os.path.join(output_dir_combined_data_error_with_regions_yearly, f"{hemisphere_chinese}_融合数据与相对误差_重点海域_{year_str}.png")
                    plot_combined_data_and_error_with_regions(yearly_fusion_avg, yearly_relative_error, x_coords_2d, y_coords_2d, proj_crs, extent,
                                                            f'{hemisphere_chinese} - {year_str}', output_path_combined_data_error_with_regions_yearly, 0, 100, seaice_cmap, year_str, sea_regions, hemisphere)
...
def main():
    # 定义14个重点海域（纬度和经度多边形）
    sea_regions = {
    'Bering Sea North': [(-180, 50), (-180, 66), (-157, 66), (-157, 50)],  # 白令海
    'Chukchi Sea North': [(-180, 66), (-180, 75), (-160, 75), (-160, 66)],  # 楚科奇海
    'East Siberian Sea North': [(100, 67), (100, 81), (140, 81), (140, 67)],  # 东西伯利亚海
    'Laptev Sea North': [(100, 70), (100, 81), (140, 81), (140, 70)],  # 拉普捷夫海
    'Kara Sea North': [(60, 68), (60, 82), (90, 82), (90, 68)],  # 喀拉海
    'Barents Sea North': [(20, 70), (20, 80), (60, 80), (60, 70)],  # 巴伦支海
    'Greenland Sea North': [(-30, 66), (-30, 80), (0, 80), (0, 66)],  # 格陵兰海
    'Baffin Bay North': [(-75, 65), (-75, 77), (-50, 77), (-50, 65)],  # 巴芬湾
    'Beaufort Sea North': [(-160, 70), (-160, 79), (-130, 79), (-130, 70)],  # 波弗特海
    'Central North': [(-170, 70), (-170, 85), (30, 85), (30, 70)],  # 中央航道 
    'Cosmonauts Sea South': [(23, -70), (23, -65), (53, -65), (53, -70)],  # 宇航员海 )
    'Prydz Bay South': [(70, -69), (70, -65), (85, -65), (85, -69)],  # 普里兹湾
    'Amundsen Sea South': [(-130, -74), (-130, -60), (-100, -60), (-100, -74)],  # 阿蒙森海
    'Ross Sea South': [(-180, -78), (-180, -60), (150, -60), (150, -78)]  # 罗斯海 
    }
    # 定义路径和变量名称
    ...
    for hemisphere in hemispheres:
        ...
        yearly_csv_path = os.path.join(results_base_path, f"{hemisphere_chinese}_年平均相对误差_12500.csv")
        hemisphere_yearly_avg.to_csv(yearly_csv_path, header=[f"{hemisphere_chinese}_年平均相对误差"],encoding='utf_8_sig')
        ...
        monthly_csv_path = os.path.join(results_base_path, f"{hemisphere_chinese}_月平均相对误差_12500.csv")
        hemisphere_monthly_avg.to_csv(monthly_csv_path, header=[f"{hemisphere_chinese}_月平均相对误差"],encoding='utf_8_sig')
        ...
        overall_csv_path = os.path.join(results_base_path, f"{hemisphere_chinese}_日相对误差_12500.csv")
        ...
        for region_name in sea_regions.keys():
            ...
            region_name_chinese = region_name.replace(" ", "").replace("North", "北极").replace("South", "南极").replace("Sea","海").replace("Ocean","洋").replace("Bay","湾").replace("Central", "中央航道").replace("Bering", "白令").replace("Chukchi", "楚科奇").replace("East Siberian", "东西伯利亚").replace("Laptev", "拉普捷夫").replace("Kara", "喀拉").replace("Barents", "巴伦支").replace("Greenland", "格林兰").replace("Baffin", "巴芬湾").replace("Beaufort", "波弗特").replace("Cosmonauts", "宇航员").replace("Prydz", "普里兹").replace("Amundsen", "阿蒙森").replace("Ross", "罗斯")
            yearly_csv_path = os.path.join(region_dir, f"{region_name_chinese}_年平均相对误差_12500.csv")
            yearly_avg.to_csv(yearly_csv_path, header=[f"{region_name_chinese}_年平均误差"], encoding='utf-8-sig')
            # print(f"Yearly averages for {region_name_chinese} saved to {yearly_csv_path}")

            monthly_csv_path = os.path.join(region_dir, f"{region_name_chinese}_月平均相对误差_12500.csv")
            monthly_avg.to_csv(monthly_csv_path, header=[f"{region_name_chinese}_月平均误差"], encoding='utf-8-sig')
