...
def kalman_filter(z, F, H, Q, R, x0, P0):
    ...
    输入：
    z - 观测值序列 （在这里为获取的卫星数据）
    F - 状态转移矩阵 （真值在在不同时刻间的函数关系）
    H - 观测矩阵（可以理解为：观测值与真值的函数关系）
    Q - 过程噪声协方差 （真值函数存在的噪声）
    R - 观测噪声协方差 （观测矩阵的噪声）
    x0 - 初始状态估计
    P0 - 初始协方差估计
    输出：
    x_estimates - 状态估计序列 （kalman输出的最优估计值）
    P_estimates - 协方差估计序列
    ...
    for k in range(T):
        x_pred = F * x_prev # 预测状态，（状态转移矩阵算出来下一刻估计值）
        P_pred = F * P_prev * F + Q # 预测协方差（下一时刻误差估计值）
...
def visualize_covariance(original, filtered, P_estimates, m, n, T):
    ...
    try:
        ...
        ax.fill_between(
            ...
            filtered - 2 * std_dev,
            filtered + 2 * std_dev,
            color="#fdae61", alpha=0.3, label="95% 置信区间"
        )
        ax.grid(True, linestyle="--", linewidth=0.5, alpha=0.7)
        ax.legend(loc="upper left")
        plt.title(f"Kalman 滤波结果-像素({m}, {n})", fontsize=15)
        plt.tight_layout()
        plt.show()
...
def apply_kalman_dataset(combined_data, Q=30, R=10, m=400, n=305):
    ...
    plt.title(f'原始数据-时间 {time_step}')
    plt.colorbar()
    plt.subplot(1, 2, 2)
    plt.imshow(smoothed[time_step], cmap='viridis')
    plt.title(f'卡尔曼滤波后数据-时间 {time_step}')
    plt.colorbar()
    plt.tight_layout()
    plt.show()
    ...
    for hemisphere in hemispheres:
        ...
        for dataset_name, config in dataset_configs.items():
            ...
            if dataset_name == 'OSI-401-d':
                ...
                if hemisphere == 'south':
                    config["var_pattern"] = "UB-AMSRE_south_icecon"
                    config["Q"] = 30
                else:
                    config["var_pattern"] = "UB-AMSRE_north_icecon"
                    config["Q"] = 30
