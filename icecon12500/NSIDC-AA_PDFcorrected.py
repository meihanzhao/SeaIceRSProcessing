...
def pdf_correction(base_input_dir, base_output_dir, base_reference_dir, hemisphere, start_date, end_date, window_size=40, time_window=30):
    ...
    # 确定非参考和参考数据变量名称和文件前缀
    if hemisphere == 'north':
        non_ref_var = 'NSIDC-AA_north_icecon'
        ref_var = 'OSI-401-d_north_icecon'
        ref_file_prefix = 'OSI-401-d_north_icecon_12500_'
        grid_x_hem, grid_y_hem = north_regular_grid()
    elif hemisphere == 'south':
        non_ref_var = 'NSIDC-AA_south_icecon'
        ref_var = 'OSI-401-d_south_icecon'
        ref_file_prefix = 'OSI-401-d_south_icecon_12500_'
    ...
    for current_date in date_range:
        ...
        # 遍历非参考数据
        non_ref_file = os.path.join(input_dir, f"{non_ref_var}_12500_{current_date.strftime('%Y%m%d')}.nc")
        ...
        output_file = os.path.join(output_dir, f"NSIDC-AA_pdfcorrected_{hemisphere}_icecon_12500_{current_date.strftime('%Y%m%d')}.nc")
        corrected_ds.to_netcdf(output_file)

        generate_snapshot(corrected_data, grid_x_hem, grid_y_hem, hemisphere, output_file, 'NSIDC-AA PDFcorrected', snapshot_folder)
        print(f"Saved corrected file: {output_file}")

if __name__ == "__main__":
    base_input_dir = r"D:\zmh\icecon12500_data\NSIDC-AA\0_pre_com"
    base_output_dir = r"D:\zmh\icecon12500_data\NSIDC-AA\1_PDFcorrected"
    base_reference_dir = r"D:\zmh\icecon12500_data\OSI-401-d\0_pre_com"
