...
def estimate_Q_R_single(data_prev, data_current, default_Q=30, default_R=10):
    ...
    Q = process_variances if not np.isnan(process_variances) else default_Q # 使用默认值
    measurement_variances = np.nanvar(data_current)
    R = measurement_variances if not np.isnan(measurement_variances) else default_R # 使用默认值
    return Q, R
...
if __name__ == "__main__":
    ...
    for hemisphere in hemispheres:
        ...
        output_base_dir = os.path.join(base_data_dir, "ICECON_merge_12500")
        ...
        for t_index, current_time in enumerate(sorted_times):
            ...
            for dataset_name, config in dataset_configs.items():
                ...
                if dataset_name == 'OSI-401-d':
                    if hemisphere == 'south':
                        adjusted_var_pattern = "OSI-401-d_south_icecon"
                        config["Q"] = 8
                    else:
                        adjusted_var_pattern = "OSI-401-d_north_icecon"
                        config["Q"] = 30
                elif dataset_name == 'OSTIA':
                    if hemisphere == 'south':
                        adjusted_var_pattern = "OSTIA_south_icecon"
                        config["Q"] = 6
                    else:
                        adjusted_var_pattern = "OSTIA_north_icecon"
                        config["Q"] = 30
                elif dataset_name == 'AMSRE':
                    if hemisphere == 'south':
                        adjusted_var_pattern = "UB-AMSRE_south_icecon"
                        config["Q"] = 30
                    else:
                        adjusted_var_pattern = "UB-AMSRE_north_icecon"
                        config["Q"] = 30 
