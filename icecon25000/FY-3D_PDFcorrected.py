...
def pdf_correction(base_input_dir, base_output_dir, base_reference_dir, hemisphere, start_date, end_date, window_size=40, time_window=30):
    ...
    for current_date in date_range:
        ...
        output_file = os.path.join(output_dir, f"FY3D_pdfcorrected_{hemisphere}_icecon_25000_{current_date.strftime('%Y%m%d')}.nc")
        corrected_ds.to_netcdf(output_file)

        generate_snapshot(corrected_data, grid_x_hem, grid_y_hem, hemisphere, output_file, 'FY3D PDFcorrected', snapshot_folder)
        print(f"Saved corrected file: {output_file}")

if __name__ == "__main__":
    base_input_dir = r"D:\zmh\icecon25000_data\FY\FY-3D\0_pre_com"
    base_output_dir = r"D:\zmh\icecon25000_data\FY\FY-3D\1_PDFcorrected"
