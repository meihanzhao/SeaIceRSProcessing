...
def pdf_correction(base_input_dir, base_output_dir, base_reference_dir, hemisphere, start_date, end_date, window_size=40, time_window=30):
    ...
    # 确定非参考和参考数据变量名称和文件前缀
    if hemisphere == 'north': 
        non_ref_var = 'cmems_sit'
        non_ref_file_prefix = 'cmems_north_SIT_25000_'
        ref_var = 'appx_sit'
    ...
    for current_date in date_range:
        ...
        corrected_ds = xr.Dataset({
            'cmems_sit': (('y', 'x'), corrected_data)
        }, coords={'x': xr.open_dataset(non_ref_file)['x'], 'y': xr.open_dataset(non_ref_file)['y']})

        output_file = os.path.join(output_dir, f"cmems_pdfcorrected_{hemisphere}_SIT_25000_{current_date.strftime('%Y%m%d')}.nc")
        corrected_ds.to_netcdf(output_file)

        generate_SIT_snapshot(corrected_ds, grid_x_n, grid_y_n, output_file, 'cmems_sit', snapshot_folder)
    
        print(f"Saved corrected file: {output_file}")

if __name__ == "__main__":
    cmems_pdf_base_input_dir = os.path.join(paths['cmems_base_folder_path'], "0_pre_com")
    cmems_pdf_base_output_dir = os.path.join(paths['cmems_base_folder_path'], "1_PDFcorrected")
    base_pdf_reference_dir = os.path.join(paths['appx_base_folder_path'], "0_pre_com")
    snapshot_folder = os.path.join(cmems_pdf_base_output_dir, "snapshots")
    ...
    for hemisphere in ["north"]:
        ...
        pdf_correction(cmems_pdf_base_input_dir, cmems_pdf_base_output_dir, base_pdf_reference_dir, hemisphere, start_date, end_date, window_size=window_size, time_window=1)