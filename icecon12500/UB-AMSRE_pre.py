...
# 文件路径 
base_folder_path = r"D:\zmh\icecon12500_data\UB-AMSRE"
    ...
    with tqdm(total=len(all_files), desc=f"Processing {hemisphere} hemisphere", unit="file") as pbar:
        for file_path in all_files:
            ...
            try:
                ...
                da_dataset = da.to_dataset(name=f'UB-AMSRE_{hemisphere}_icecon')  
                output_file = os.path.join(
                    output_folder,
                    f'UB-AMSRE_{hemisphere}_icecon_12500_{os.path.basename(file_path)[-16:-8]}.nc'  # 输出文件名
                )
                generate_snapshot(grid_data, grid_x_hem, grid_y_hem, hemisphere, output_file, 'UB-AMSRE', snapshot_folder)
