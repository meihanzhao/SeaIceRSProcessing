...
# 文件路径
base_folder_path = r"D:\zmh\icecon25000_data\FY\FY-3D"
...
def process_hemisphere(hemisphere: str):
    ...
    if hemisphere == 'north':
        ...
        output_variable = 'FY3D_north_icecon'
        lat_filter = 60
        ...
        output_variable = 'FY3D_south_icecon'
        lat_filter = -60
        lat_condition = lambda lat: lat <= lat_filter
    else:
    ...
    with tqdm(total=len(all_files), desc=f"Processing {hemisphere}", unit="file") as pbar:
        for file_path in all_files:
            ...
            try:
                ...
                generate_snapshot(grid_data, grid_x_hem, grid_y_hem, hemisphere, output_file, 'FY3D', snapshot_folder)
