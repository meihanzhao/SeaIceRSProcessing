...
def read_data_single(file_path, var_name_pattern):
    ...
    if var_name is None:
        ds.close()
        raise ValueError(f"No variable found in file matching pattern '{var_name_pattern}'") # 修改点：不再输出报错信息，仅抛出异常
...
def process_dataset_at_time(dataset_name, config, hemisphere, current_time, t_index, x_prev_dict, P_prev_dict, base_data_dir, output_base_dir, x, y):
    ...
    try:
        ...
        # 修改点：不再输出报错信息，仅打印跳过信息
        print(f"    Skipping file {current_file} due to error: No variable found in file matching pattern '{adjusted_var_pattern}'")
        return None, None
...
def fuse_and_save_data(estimated_values, uncertainty_list, current_time, output_base_dir, hemisphere, x, y):
    ...
    if estimated_values:
        ...
        combined_values, combined_uncertainty = calculate_combined_uncertainty(
            estimated_values, uncertainty_list, y_len, x_len  # Pass y_len and x_len here
        )
...
def main():
    ...
    dataset_configs = {
        ...
        "NSIDC-Bootstrap": {"path": "NSIDC-Bootstrap/0_pre_com", "var_pattern": "NSIDC-BS", "Q": 30, "R": 10}, # 修改点: var_pattern 更通用，并在process_dataset_at_time中调整
    ...
    for hemisphere in hemispheres:
        ...
        output_base_dir = os.path.join(base_data_dir, "ICECON_merge_25000_test")

        # 得到所有时间点和每个时间点可用的数据集 # 修改点1：记录每个时间点可用的数据集
        ...
        for dataset_name, config in dataset_configs.items():
            ...
            for file in files:
                try:
                    ...
                    all_times.add(time)
                    if time not in dataset_availability:
                        dataset_availability[time] = []
                    dataset_availability[time].append(dataset_name)
                except ValueError as e:
                    if "No variable found in file matching pattern" not in str(e): # 修改点2: 仅当不是变量找不到的错误时才输出，或者可以完全静默
                        print(f"Skipping file {file} due to error: {e}")
                    else:
                        print(f"Skipping file {file} due to error: {e}") # 保留此打印，用于调试目的
        ...
        for t_index, current_time in enumerate(sorted_times):
            ...
            available_datasets_at_time = dataset_availability.get(current_time, []) # 修改点3：获取当前时间可用的数据集列表

            # 处理每个数据集
            for dataset_name in dataset_configs: # 修改点4：遍历所有数据集配置，但只处理当前时间可用的数据集
                if dataset_name in available_datasets_at_time: # 修改点5：检查数据集是否在当前时间可用
                    ...
                    print(f"  Skipping dataset: {dataset_name} at {current_time} as no data is available.") # 修改点6：打印跳过信息

            # 融合数据，只有当estimated_values不为空时才进行融合
            if estimated_values: # 修改点7：只有当有可用数据集时才融合
                fuse_and_save_data(estimated_values, uncertainty_list, current_time, output_base_dir, hemisphere, x, y)
            else:
                print(f"No datasets available for fusion at {current_time}. Skipping fusion.") # 修改点8：打印无数据融合信息

            del estimated_values, uncertainty_list

    # --- 画图 ---
    data_dir = os.path.join(base_data_dir, "ICECON_merge_25000_test")
    for hemisphere in hemispheres:
