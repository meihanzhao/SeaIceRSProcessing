...
grids = xr.open_dataset(r"D:\zmh\icecon12500_data\RegularGrid\grid-n6250.nc")
grids_s = xr.open_dataset(r"D:\zmh\icecon12500_data\RegularGrid\grid-s6250.nc")

# 定义北极规则网格
def north_regular_grid():
    grid_x_n, grid_y_n = np.meshgrid(
        np.arange(grids['x'].values.min(), grids['x'].values.max(), 12500),
        np.arange(grids['y'].values.min(), grids['y'].values.max(), 12500)
    )
...
def south_regular_grid():
    grid_x_s, grid_y_s = np.meshgrid(
        np.arange(grids_s['x'].values.min(), grids_s['x'].values.max(), 12500),
        np.arange(grids_s['y'].values.min(), grids_s['y'].values.max(), 12500)
    )
