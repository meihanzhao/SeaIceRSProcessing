...
# 文件路径
PIOMAS_base_folder_path = paths['PIOMAS_base_folder_path']
output_folder_north = os.path.join(PIOMAS_base_folder_path, '0_pre_com', 'north')
failure_folder = os.path.join(PIOMAS_base_folder_path, '0_pre_com', 'failures')
snapshot_folder = os.path.join(PIOMAS_base_folder_path, "0_pre_com", "snapshots")
...
def process_north():
    ...
    for year in range(2004, 2024):  # 遍历年份
        year_folder_path = os.path.join(PIOMAS_base_folder_path, str(year))  # 年份文件夹路径
    ...
    with tqdm(total=len(all_files), desc="Processing files") as pbar:
        for file_path in all_files:  # 测试
            try:
                ...
                # print(sit_data)
                sit_data = sit_data[['thickness', 'longitude', 'latitude']].drop('time')  # 仅保留海冰厚度、经度和纬度数据
                sit_data['thickness'] = sit_data['thickness'].squeeze()  # 海冰厚度数据
                ...
                x, y = transformer.transform(sit_data['longitude'].values, sit_data['latitude'].values)
                # 插值到规则网格
                sit_data_interpolated = griddata(
                    (x.flatten(), y.flatten()),
                    sit_data['thickness'].values.flatten(),
                    (grid_x_n, grid_y_n),
                    method='nearest'
                )
                ...
                da_dataset = da.to_dataset(name='PIOMAS_sit')
                output_filename = os.path.join(
                    output_folder_north, 
                    f'PIOMAS_north_SIT_25000_{os.path.basename(file_path)[-11:-3]}.nc'  # 生成文件名
                )
                # 生成快视图
                generate_SIT_snapshot(da_dataset, grid_x_n, grid_y_n, output_filename, 'PIOMAS_sit', snapshot_folder)
