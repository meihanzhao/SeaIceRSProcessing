...
def plot_combined_comparison_and_error(data1, data2, relative_error, x_coords, y_coords, proj_crs, extent, title, output_path, vmin, vmax, cmap, date_str=None):
    ...
    try:
        ...
        error_colors = ["#f0f9e8", "#bae4bc", "#7bccc4", "#43a2ca", "#0868ac", "#084081"]
        error_cmap = LinearSegmentedColormap.from_list("SeaIceCmap", error_colors)
        error_bounds = [0, 10, 20, 30, 40, 50, 60, 70, 80]
...
def process_files(fusion_path, test_path, var_fusion, var_test, hemisphere, sea_regions, results_base_path):
    ...
    for fusion_file in fusion_files:
        ...
        fusion_datasets[date_str] = os.path.join(fusion_path, fusion_file)

    # 加载检验数据
    for test_file in test_files:
        date_str = test_file[-11:-3]
        test_datasets[date_str] = os.path.join(test_path, test_file)
    
    # 得到相同的数据字符串
    common_dates = sorted(list(set(fusion_datasets.keys()) & set(test_datasets.keys())))
    
    # 仅处理相同的数据
    for date_str in common_dates:
        try:
            fusion_ds = xr.open_dataset(fusion_datasets[date_str])
            test_ds = xr.open_dataset(test_datasets[date_str])
        ...
        if date.day == 1:
            ...
            for next_day in range(1, 32):
                try:
                    ...
                    next_fusion_file_path = fusion_datasets.get(next_date_str)
                    next_test_file_path = test_datasets.get(next_date_str)
                    
                    if next_fusion_file_path is None or next_test_file_path is None:
                        print(f"Skipping missing file for {next_date_str}")
                        continue

                    next_fusion_ds = xr.open_dataset(next_fusion_file_path)
                    next_test_ds = xr.open_dataset(next_test_file_path)
                    ...
                    print(f"Skipping missing file or variable for {next_date_str}: {e}")
                    continue
                ...
                if date.month == 1:
                    ...
                    for next_month in range(1, 12):
                        try:
                            ...
                            print(f"Skipping missing files for month {next_month_date.strftime('%Y%m')}: {e}")
                            break
...
def main():
    ...
    # 定义路径和变量名称
    fusion_base_path = "D:\\zmh\\icecon25000_data\\ICECON_merge_25000_kalmanV1.3"
    results_base_path = "D:\\zmh\\icecon25000_data\\ICECON_merge_25000_kalmanV1.3\\error_results_test"
    os.makedirs(results_base_path, exist_ok=True)
    
    test_base_paths = {
        'north': "D:\\zmh\\icecon25000_data\\OSI-401-d\\0_pre_com\\north",
        'south': "D:\\zmh\\icecon25000_data\\OSI-401-d\\0_pre_com\\south"
    }
    hemispheres = ['south', 'north']
    fusion_var_names = {
        'north': 'ice_con',
        'south': 'ice_con'
    }
    test_var_names = {
        'north': 'OSI-401-d_north_icecon',
        'south': 'OSI-401-d_south_icecon'
    }
    ...
    for hemisphere in hemispheres:
        ...
        fusion_path = os.path.join(fusion_base_path, hemisphere, 'Icecon_Combined')
        ...
        yearly_csv_path = os.path.join(results_base_path, f"{hemisphere_chinese}_年平均相对误差_25000.csv")
        ...
        monthly_csv_path = os.path.join(results_base_path, f"{hemisphere_chinese}_月平均相对误差_25000.csv")
        ...
        overall_avg_path = os.path.join(results_base_path, f"{hemisphere_chinese}_总平均相对误差_25000.txt")
        ...
        overall_csv_path = os.path.join(results_base_path, f"{hemisphere_chinese}_日相对误差_25000.csv")
        ...
        for region_name in sea_regions.keys():
            ...
            yearly_csv_path = os.path.join(region_dir, f"{region_name_chinese}_年平均相对误差_25000.csv")
            ...
            monthly_csv_path = os.path.join(region_dir, f"{region_name_chinese}_月平均相对误差_25000.csv")
            ...
            overall_avg_path = os.path.join(region_dir, f"{region_name_chinese}_总平均相对误差_25000.txt")
            with open(overall_avg_path, 'w', encoding='utf-8') as f:
