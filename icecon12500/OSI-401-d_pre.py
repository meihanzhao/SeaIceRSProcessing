...
# 文件路径
base_folder_path = r"D:\zmh\icecon12500_data\OSI-401-d"
...
def process_hemisphere(hemisphere: str, start_year: int, end_year: int):
    ...
    if hemisphere == 'north':
        ...
        failure_folder = os.path.join(base_folder_path, "0_pre_com", "failures","north")
        ...
        failure_folder = os.path.join(base_folder_path, "0_pre_com", "failures","south")
        mask_condition = lambda lat: lat <= -60
            ...
            with tqdm(total=len(all_files), desc=f"Processing {hemisphere} hemisphere {year}-{month:02d}", unit="file") as pbar:
                for sic_file in all_files:
                    ...
                    output_filename = os.path.join(
                        output_folder,
                        f'OSI-401-d_{hemisphere}_icecon_12500_{os.path.basename(sic_file)[-15:-7]}.nc'
                    )
