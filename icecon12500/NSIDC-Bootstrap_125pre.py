...
# 文件路径 
base_folder_path = r"D:\zmh\icecon25000_data\NSIDC-Bootstrap"
output_folder_north = os.path.join(base_folder_path, '0_pre_com_125', 'north') 
output_folder_south = os.path.join(base_folder_path, '0_pre_com_125', 'south')  
failure_folder = os.path.join(base_folder_path, '0_pre_com_125', 'failures')
snapshot_folder = os.path.join(base_folder_path, "0_pre_com_125", "snapshots")  
...
def process_hemisphere(hemisphere: str):
    ...
    # 遍历每个年份的数据
    all_files = []  # 存储所有文件的列表
    for year in range(1984, 2024):  # 遍历年份
    ...
    with tqdm(total=len(all_files), desc=f"Processing {hemisphere} hemisphere", unit="file") as pbar:
        for file_path in all_files:
            ...
            try:
                ...
                ds_sic = ds_sic.drop_vars('crs')  # 删除多余变量
                ds_sic = ds_sic.drop('time')
                ds_sic['F17_ICECON'] = ds_sic['F17_ICECON'].squeeze()
                ds_sic = ds_sic.rename_vars({'F17_ICECON': 'BS_icecon'})
                ...
                mask1 = (ds_sic['BS_icecon']>=0) & (ds_sic['BS_icecon']<=1)
                ds_sic = ds_sic.where(mask1, drop=False)
                ds_sic['BS_icecon'] = ds_sic['BS_icecon']*100

                # --- 步骤3：插值到目标网格 ---
                x_flat = x_grid.flatten()  
                y_flat = y_grid.flatten()  
                z_flat = ds_sic['BS_icecon'].values.flatten()  
                ...
                da_dataset = da.to_dataset(name=f'NSIDC-BS_{hemisphere}_icecon')  
                output_file = os.path.join(
                    output_folder,
                    f'NSIDC-BS_{hemisphere}_icecon_12500_{os.path.basename(file_path)[-16:-8]}.nc'  # 输出文件名
                )
                generate_snapshot(grid_data, grid_x_hem, grid_y_hem, hemisphere, output_file, 'NSIDC-BS', snapshot_folder)
